<template>
  <section>
    <h5 class="mb-3">
      {{ $route.name === "EditAd" ? "Отредактируйте фото" : "Добавьте фото" }}
      <small class="mb-3 text-muted"
        >(c фото ваше объявление будет заметнее)</small
      >
    </h5>
    <div class="d-flex">
      <input
      ...
      <div class="create-add-form__images mb-3">
        <!-- LOADED-images -->
        <div class="loadedImages d-flex flex-wrap">
          <span
          ...
          >
            <button
              type="button"
              class="btn-close position-absolute right-1"
              style="top: 0.5rem"
            ...
            <img
              ...
              :class="{
                'border border-success border-3':
                  defaultMainImage === i + isloadedImagesLength,
              }"
            />
          </span>
          <button
